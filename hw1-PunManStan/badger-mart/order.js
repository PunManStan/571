...
function calculateSubtotal() {
	...
	for(let i = 0;i<ITEMS.length; i++){
		let itemQuant = ITEMS[i] + "-quantity";
		let itemprice = ITEMS[i] + "-price";
		let quantity = parseInt(document.getElementById(itemQuant).value);
		let price = parseFloat(document.getElementById(itemprice).innerText);
...
function calculateSalesTax() {
	...
	let salesTax = roundMoney(tax*subtotal);
	return salesTax; 
}
...
document.getElementById("btn-checkout").addEventListener("click", () => {
	alert("Your total is: $" + (calculateSalesTax()+calculateSubtotal()).toFixed(2));
});
